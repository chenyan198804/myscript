...
from CreditCard.conf import Templates
...
def print_shopping_history(userobj):
    ...
    history_list = dbapi.load_shop_histroy(userobj.username, start, end)
    # 获取模板文件样式
    _Templates = Templates.shopping_history
    common.show_message(_Templates.format(username=userobj.username,
                                         startdate=start,
                                         enddate=end), "NOTICE")
...
def print_bill_history(userobj):
    ...
    # 填充模板并打印
    common.show_message(_Templates.format(cardno=userobj.bindcard,
                                         startdate=startdate,
                                         enddate=enddate,
                                         billdetail="\n".join(msglist)
                                         ), "NOTICE")
...
def print_statement_list(cardno, list_info):
    ...
    """
    # 获取显示模板
    show_Templates = Templates.report_statement_list
    tmpstrlist = list()
    ...
    if len(list_info) > 0:
        ...
        # 填充模板,将填充后的魔板信息返回
        result = show_Templates.format(cardno=cardno, show_msg="\n".join(tmpstrlist))
        return result
    else:
        return ""
