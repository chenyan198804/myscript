#!/usr/bin/env python
#_*_coding:utf-8_*_
import hashlib, random,os,time,datetime,logging
...
def create_serialno():
    ...
    :return: 流水号
    """
    serno = "{0}{1}".format(datetime.datetime.now().strftime("%Y%m%d%H%M%S"), str(int(time.time())))
    return serno
    ...
    while is_null_flag:
        ...
        if not input_value:
            ...
            if input_value not in limit_value:
                show_message('输入值不正确，请重新输入！','ERROR')
                continue
            else:
                is_null_flag = False
...
def input_date(msg,default_date):
    ...
    while not check_flag:
        ...
        try:
            ...
            result = datetime.date(int(date_list[0]),int(date_list[1]),int(date_list[2]))
            check_flag = True
